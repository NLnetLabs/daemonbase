...
impl ConfigFile {
    ...
    pub fn take_value(&mut self, key: &str) -> Result<Option<toml::Value>, Failed> {
    ...
    pub fn take_u64(&mut self, key: &str) -> Result<Option<u64>, Failed> {
        match self.take_value(key)? {
            Some(toml::Value::Integer(value)) => match u64::try_from(value.into_value()) {
    ...
    pub fn take_limited_u8(&mut self, key: &str, limit: u8) -> Result<Option<u8>, Failed> {
    ...
    pub fn take_small_usize(&mut self, key: &str) -> Result<Option<usize>, Failed> {
    ...
    /// is no such key. Returns an error if the key exists but the value
    /// isn’t a string.
    pub fn take_string(&mut self, key: &str) -> Result<Option<String>, Failed> {
        match self.take_value(key)? {
    ...
    pub fn take_path(&mut self, key: &str) -> Result<Option<ConfigPath>, Failed> {
    ...
    pub fn take_mandatory_path(&mut self, key: &str) -> Result<ConfigPath, Failed> {
    ...
    pub fn take_string_array(&mut self, key: &str) -> Result<Option<Vec<String>>, Failed> {
        match self.take_value(key)? {
    ...
    /// returns an error.
    pub fn take_from_str_array<T>(&mut self, key: &str) -> Result<Option<Vec<T>>, Failed>
    where
        T: FromStr,
        T::Err: fmt::Display,
    {
    ...
    pub fn take_path_array(&mut self, key: &str) -> Result<Option<Vec<ConfigPath>>, Failed> {
        match self.take_value(key)? {
        ...
        match self.take_value(key)? {
            Some(toml::Value::Array(vec)) => {
                ...
                for value in vec.into_iter() {
                    ...
                    if res.insert(left.into_value(), right.into_value()).is_some() {
                        error!(
                            "Failed in config file {}: \
                            'duplicate item in '{}'.",
                            self.path.display(),
    ...
    pub fn insert_string(&mut self, key: &str, value: impl ToString) {
        self.content.insert(
            key,
            toml::Item::Value(toml::Value::String(toml::Formatted::new(value.to_string()))),
        );
    }
...
impl clap::builder::TypedValueParser for ConfigPathParser {
    ...
    fn parse_ref(
        ...
        let path = clap::builder::PathBufValueParser::new().parse_ref(cmd, arg, value)?;
        if path.is_absolute() {
        ...
        let cur_dir = match env::current_dir() {
            Ok(dir) => dir,
            Err(err) => {
                let mut res = clap::Error::new(clap::error::ErrorKind::Io).with_cmd(cmd);
                res.insert(
