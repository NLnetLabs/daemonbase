    ...
    pub fn from_config(config: &Config) -> Result<Self, Failed> {
        Ok(Self {
            ...
            target: match config.log_target {
                ...
                #[cfg(unix)]
                TargetName::Syslog => Target::Syslog(config.syslog_facility.into()),
                TargetName::Stderr => Target::Stderr,
                TargetName::File => match config.log_file.as_ref() {
                    Some(LogPath::Stderr) => Target::Stderr,
                    Some(LogPath::Path(file)) => Target::File(file.clone().into()),
                    None => {
...
impl Config {
    ...
    pub fn from_config_file(file: &mut ConfigFile) -> Result<Self, Failed> {
        Ok(Self {
            log_level: file
                ...
                .unwrap_or_default(),
            log_target: file.take_from_str::<TargetName>("log")?.unwrap_or_default(),
            #[cfg(unix)]
    ...
    pub fn add_to_config_file(&self, config: &mut ConfigFile) {
        ...
        if !self.syslog_facility.is_default() {
            config.insert_string("syslog-facility", self.syslog_facility.as_str());
        }
...
impl PartialEq for Target {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            #[cfg(unix)]
            (&Self::Syslog(s), &Self::Syslog(o)) => (s as usize) == (o as usize),
...
impl Dispatch {
    ...
    fn new(config: &Logger, daemon: bool) -> Result<Self, Failed> {
        let target = match config.target {
            ...
            Target::Default => {
                if daemon {
                    Self::new_syslog_target(syslog::Facility::LOG_DAEMON, false)?
                } else {
            ...
            #[cfg(unix)]
            Target::Syslog(facility) => Self::new_syslog_target(facility, true)?,
    ...
    #[cfg(unix)]
    fn new_syslog_target(facility: syslog::Facility, use_inet: bool) -> Result<LogBackend, Failed> {
...
mod unix {
    ...
    pub struct SyslogLogger(syslog::Logger<syslog::LoggerBackend, syslog::Formatter3164>);

    impl SyslogLogger {
        /// Creates a new syslog logger.
        pub fn new(facility: syslog::Facility, use_inet: bool) -> Result<Self, Failed> {
