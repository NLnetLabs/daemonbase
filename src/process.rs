...
#[cfg(unix)]
pub use self::unix::{exit_signalled, Args, Config, GroupId, Process, UserId};

#[cfg(target_os = "linux")]
pub use self::linux::EnvSockets;

#[cfg(not(unix))]
pub use self::not_unix::{exit_signalled, Args, Config, Process};
...
mod unix {
    ...
    ///
    pub async fn exit_signalled() -> io::Result<()> {
